/* This file was generated by SableCC (http://www.sablecc.org/). */

package jah.node;

import jah.analysis.*;

@SuppressWarnings("nls")
public final class AComandoAvalieComm extends PComm
{
    private TAvalie _avalie_;
    private TAbreParentesis _abreParentesis_;
    private PExp _exp_;
    private TFechaParentesis _fechaParentesis_;
    private PAvalieSenao _avalieSenao_;
    private PFimAvalie _fimAvalie_;

    public AComandoAvalieComm()
    {
        // Constructor
    }

    public AComandoAvalieComm(
        @SuppressWarnings("hiding") TAvalie _avalie_,
        @SuppressWarnings("hiding") TAbreParentesis _abreParentesis_,
        @SuppressWarnings("hiding") PExp _exp_,
        @SuppressWarnings("hiding") TFechaParentesis _fechaParentesis_,
        @SuppressWarnings("hiding") PAvalieSenao _avalieSenao_,
        @SuppressWarnings("hiding") PFimAvalie _fimAvalie_)
    {
        // Constructor
        setAvalie(_avalie_);

        setAbreParentesis(_abreParentesis_);

        setExp(_exp_);

        setFechaParentesis(_fechaParentesis_);

        setAvalieSenao(_avalieSenao_);

        setFimAvalie(_fimAvalie_);

    }

    @Override
    public Object clone()
    {
        return new AComandoAvalieComm(
            cloneNode(this._avalie_),
            cloneNode(this._abreParentesis_),
            cloneNode(this._exp_),
            cloneNode(this._fechaParentesis_),
            cloneNode(this._avalieSenao_),
            cloneNode(this._fimAvalie_));
    }

    @Override
    public void apply(Switch sw)
    {
        ((Analysis) sw).caseAComandoAvalieComm(this);
    }

    public TAvalie getAvalie()
    {
        return this._avalie_;
    }

    public void setAvalie(TAvalie node)
    {
        if(this._avalie_ != null)
        {
            this._avalie_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._avalie_ = node;
    }

    public TAbreParentesis getAbreParentesis()
    {
        return this._abreParentesis_;
    }

    public void setAbreParentesis(TAbreParentesis node)
    {
        if(this._abreParentesis_ != null)
        {
            this._abreParentesis_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._abreParentesis_ = node;
    }

    public PExp getExp()
    {
        return this._exp_;
    }

    public void setExp(PExp node)
    {
        if(this._exp_ != null)
        {
            this._exp_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._exp_ = node;
    }

    public TFechaParentesis getFechaParentesis()
    {
        return this._fechaParentesis_;
    }

    public void setFechaParentesis(TFechaParentesis node)
    {
        if(this._fechaParentesis_ != null)
        {
            this._fechaParentesis_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._fechaParentesis_ = node;
    }

    public PAvalieSenao getAvalieSenao()
    {
        return this._avalieSenao_;
    }

    public void setAvalieSenao(PAvalieSenao node)
    {
        if(this._avalieSenao_ != null)
        {
            this._avalieSenao_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._avalieSenao_ = node;
    }

    public PFimAvalie getFimAvalie()
    {
        return this._fimAvalie_;
    }

    public void setFimAvalie(PFimAvalie node)
    {
        if(this._fimAvalie_ != null)
        {
            this._fimAvalie_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._fimAvalie_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._avalie_)
            + toString(this._abreParentesis_)
            + toString(this._exp_)
            + toString(this._fechaParentesis_)
            + toString(this._avalieSenao_)
            + toString(this._fimAvalie_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._avalie_ == child)
        {
            this._avalie_ = null;
            return;
        }

        if(this._abreParentesis_ == child)
        {
            this._abreParentesis_ = null;
            return;
        }

        if(this._exp_ == child)
        {
            this._exp_ = null;
            return;
        }

        if(this._fechaParentesis_ == child)
        {
            this._fechaParentesis_ = null;
            return;
        }

        if(this._avalieSenao_ == child)
        {
            this._avalieSenao_ = null;
            return;
        }

        if(this._fimAvalie_ == child)
        {
            this._fimAvalie_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._avalie_ == oldChild)
        {
            setAvalie((TAvalie) newChild);
            return;
        }

        if(this._abreParentesis_ == oldChild)
        {
            setAbreParentesis((TAbreParentesis) newChild);
            return;
        }

        if(this._exp_ == oldChild)
        {
            setExp((PExp) newChild);
            return;
        }

        if(this._fechaParentesis_ == oldChild)
        {
            setFechaParentesis((TFechaParentesis) newChild);
            return;
        }

        if(this._avalieSenao_ == oldChild)
        {
            setAvalieSenao((PAvalieSenao) newChild);
            return;
        }

        if(this._fimAvalie_ == oldChild)
        {
            setFimAvalie((PFimAvalie) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
